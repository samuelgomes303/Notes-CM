...
class UpdateFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        view.findViewById<TextView>(R.id.updateNote).text = args.currentNote.descricao
        view.findViewById<TextView>(R.id.updateDate).text = args.currentNote.data
    ...
    private  fun updateNote(){
        ...
        val dataEditText = view?.findViewById<EditText>(R.id.updateDate)?.text.toString()

        if(noteText.isEmpty()) {
            makeText(context , "Não pode uma nota vazia!", Toast.LENGTH_LONG).show()
        } else if (dataEditText.isNotEmpty()) {

            val note = Note(args.currentNote.id, noteText, dataEditText)
